...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, avatar ".//会员头像
    ...
    $info['avatar']      = isset($infos['avatar']) ? $infos['avatar'] : '';//会员头像
...
function get_consignee_list($user_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id'  order by address_id desc LIMIT  10 ";

    return $GLOBALS['db']->getAll($sql);
}


function get_consignee_info($user_id,$address_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, shipping_id,invoice_no,order_status, shipping_status, pay_status, add_time, " .
 ...
 */
function get_order_detail($order_id, $user_id = 0, $agent_type)
{
 ...
 //             $order['invoice_no'] = $shipping->query($order['invoice_no']);
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            $order['pay_online'] = $pay_obj->get_code($order, $payment,$agent_type);
        }
    }
    else
    {
    ...
    if(!empty($order['best_time'])){
          $bt = explode(' ',$order['best_time']);
          if(count($bt)>1){
              $order['best_time'] = $bt[0];
              $order['best_time1'] = $bt[1];
          }
    }
    return $order;
