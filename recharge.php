...
//-- 充值
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'recharge')
{

    include_once('includes/cls_json.php');
    
    //var GoodsForm=new array();
    //
    //var GoodsForm = new FormData();
    //GoodsForm.append("quick", 1);
    //GoodsForm.append("spec", []);
    //GoodsForm.append("goods_id", $_GET["id"]);
    //GoodsForm.append("number", 1);
    //GoodsForm.append("parent", 0);
    //echo GoodsForm;
    //die;
    //$_POST['goods'] = json_str_iconv($_POST['goods']);
    $id=$_GET["id"];
    $_POST['goods']="{\"quick\":1,\"spec\":[],\"goods_id\":$id,\"number\":\"1\",\"parent\":0}";
//echo $_POST['goods'];
//die;
...
clear_cart();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        if(!$notvirtual){
            //$smarty->display('exchange.dwt');
            ecs_header("Location: recharge.php?step=checkout\n");
            exit;
        }
   }
}   
...
elseif ($_REQUEST['step'] == 'done')
{
  ...
  $cart_goods[$key]['goods_img'] = $goods_tmp['goods_img'];
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        $pay_online = $pay_obj->get_code($order, unserialize_config($payment['pay_config']),'07');
