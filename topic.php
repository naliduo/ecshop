...
if (!$smarty->is_cached($templates, $cache_id))
{
    ...
    $arr = (array)$tmp;
    if($topic["cat_img"]){
        $catimage =  @unserialize($topic["cat_img"]);
        $catimage = (array)$catimage;
        $smarty->assign('catimage',      $catimage);
    }
    $goods_id = array();
    ...
    $smarty->assign('sort_goods_arr',   $arr);          // 商品列表
