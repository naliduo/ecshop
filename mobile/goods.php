...
$goods_info['commengp'] = 0;
//查询是否购买过商品
if($_SESSION['user_id']){
    $commengp = $db->getOne("select count(*) from ".$ecs->table("order_info")." o,".$ecs->table("order_goods")." g where o.order_id = g.order_id and g.goods_id=".$goods_id." and o.user_id=".$_SESSION['user_id']);
    $goods_info['commengp'] = $commengp;
}
...
if ($act == 'view_riji')
{
	$goods_riji = str_replace("src=\"/images", "src=\"../images", $goods_info['riji']);
	$smarty->assign('goods_riji' , $goods_riji);
	$smarty->display('goods_riji.dwt');
	exit();
}

if ($act == 'view_ways')
{
	$goods_ways = str_replace("src=\"/images", "src=\"../images", $goods_info['ways']);
	$smarty->assign('goods_ways' , $goods_ways);
	$smarty->display('goods_ways.dwt');
	exit();
}
