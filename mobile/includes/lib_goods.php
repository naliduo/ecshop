    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            ...
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ...
               ' WHERE g.is_on_sale = 1 '.$where.' AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
		...
		$bindusercardme =  $GLOBALS['smarty']->get_template_vars("bindusercardme");
        if(empty($_SESSION['user_id'])){
			 $sql .= " and g.is_bind_card!='2'  ";
		}
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            " AND g.is_promote = 1  $where AND promote_start_date <= '$time' AND promote_end_date >= '$time' ";
