...
if($act == 'checkout' || $act == 'consignee'){
	...
	if($act == 'consignee')
	{
        ...
        $smarty->assign('real_goods_count', $real_goods_count);
        if(!$real_goods_count){
        ...
        if(count($consignee_list)==0){
            ecs_header("Location: user.php?act=add_address\n");
            exit;
        }
		/* 取得每个收货地址的省市区列表 */
		$province_list = array();
        ...
        $smarty->assign('addressid', $db->getOne("select address_id from ".$ecs->table("user_address")." where user_id=".$_SESSION['user_id']));
		$smarty->assign('buy_type', 1);

	}
}

else if($act == 'checkout2'){
	if ($_SESSION['user_id'] > 0)
	{
		$act = 'consignee';
	}
	if($act == 'consignee')
	{
        ...
        $smarty->assign('real_goods_count', $real_goods_count);
        if(!$real_goods_count){
            ecs_header("Location: order.php?act=order_lise&noshowlist\n");
            exit;
