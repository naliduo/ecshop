...
if($real_goods_count){
   ...
   $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
    ...
    if(!empty($u['to_date']) &&  $u['to_date']<gmtime()){ //判断合同期限
        ...
        if($u['user_money']==0){
            $smarty->assign('msg', '您的订单无法提交，您的会员卡已超过合同期。<a href=category.php?c_id=9 target=_blank>去充值</a>');
        }else{
            $smarty->assign('msg', '很抱歉，您的会员资格已过了合约有效期（相关规定请参照网站首页-购物指南-<a 

href=article.php?act=detail&a_id=23 target=_blank>会员卡协议</a>），您账户内的余额和

等级分仍为您保留，但目前无法继续订购。

您现可以选择：
1）联系客服400-821-3860，申请享受和您所购会员卡相应的延长期的权利。
2）如您已享受完单次的延长期服务，通过客服协助，您仍可以注册会员身份继续

用账户内余额购物，但不再享有原会员等级的优惠服务。取消的原等级分仍将被保

留一年的记录，以便您一年中再续费时累积计入您的等级分中 。
3）直接购卡<a href=category.php?c_id=9 target=_blank>续费</a>，并在用户

中心-会员卡栏目绑定操作后，即可继续享受相应的会员等级优惠服务。
');
        }
    }
...
elseif ($_REQUEST['act'] == 'check_surplus')
{
    /*------------------------------------------------------ */
    //-- 检查用户输入的余额
    /*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'check_integral')
{
    /*------------------------------------------------------ */
    //-- 检查用户输入的余额
    /*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'select_payment')
{
    ...
    include_once('includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'content' => '', 'need_insure' => 0, 'payment' => 1, 'msg'=>0, 'virtual_card'=>0);
	...
	{
		if($goods['extension_code']=='virtual_card') $result['virtual_card'] = 1;
		
	}
	
	if($result['virtual_card'])
	{
		if(intval($_REQUEST['payment'])==3)
		{
			$result['msg'] = 0;
		}
		else
...
//    die(sprintf($_LANG['bonus_is_ok'], price_format($bonus['type_money'], false)));
    ...
    else
    {
        ...
        if($total['goods_price']<$bonus['min_goods_amount'])
        {
         ...
         /* 重新计算订单 */
         $total = order_fee($order, $cart_goods, $consignee);
         $result['error'] = sprintf('订单商品金额没有达到使用该抵扣券的最低金额 %s', price_format($bonus['min_goods_amount'], false));
        }
...
elseif($_REQUEST['act'] = 'done')
{
    ...
    if ($flow_type == CART_GENERAL_GOODS && cart_amount(true, CART_GENERAL_GOODS) < $_CFG['min_goods_amount'])
    {
        ...
        $tips = "您购买的商品没有达到本店的最低限购金额 ￥".$_CFG['min_goods_amount']."元 ，不能提交订单。<br/><a href='javascript:history.back()'>返回上一页</a>";
        $smarty->assign('footer', get_footer());
	...
	/* 如果订单金额为0（使用余额或积分或抵扣券支付），修改订单状态为已确认、已付款 */
	...
	if ($_CFG['ihuyi_sms_customer_placed'] == '1' && $row['mobile_phone'] != '')
	{
		...
		$smarty->assign('best_time',	$order['best_time']);
	
		$content = $smarty->fetch('str:' . $_CFG['ihuyi_sms_customer_placed_value']);
	
		$ret = sendsms($order['tel'], $content);
	}
	...
	if ($order['order_amount'] > 0)
	{
		...
		$pay_online = $pay_obj->get_code($order, unserialize_config($payment['pay_config']),'08');

		$order['pay_desc'] = $payment['pay_desc'];

		// Add by Edward
		if ( ($payment['pay_code'] == 'alipay')&&(strpos($_SERVER['HTTP_USER_AGENT'], "MicroMessenge"))) {
			$smarty->assign('pay_online', "<p>您的订单已提交，由于支付宝封闭微信渠道的缘故，您可以：</p><p>1、请扫描该二维码支付。</p><p>如何支付：请将此二维码保存在手机相册中，并用支付宝客户端扫一扫功能：用扫一扫页面顶端的图片标，点击即可选择手机相册的二维码，自动导入 支付宝支付页面即可完成该订单的支付。支付完毕请联系400-821-3860客户热线为您核对订单。</p>");
			$smarty->assign('alipay_img','1');
			$smarty->assign('alipaydr_online','<p>2.直接在手机浏览器上访问君禾园WAP版网站shop.dream-gardens.com.cn进入用户中心--我的订单进行支付。</p>');
