...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $from_date = empty($_POST['from_date']) ? '' : local_strtotime($_POST['from_date']." 00:00:00");
    $to_date = empty($_POST['to_date']) ? '' : local_strtotime($_POST['to_date']." 23:59:59");
    $users =& init_users();
    ...
    $other['dinggou_way']   = $_POST['dinggou_way'];
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.dinggou_way,u.sex, u.birthday, u.pay_points, u.from_date,u.to_date,u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
    ...
    $sql = "SELECT u.user_id, u.dinggou_way,u.sex, u.birthday, u.pay_points, u.rank_points,u.from_date,u.to_date, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
    ...
    if ($row)
    {
        ...
        $user['dinggou_way']        = $row['dinggou_way'];
        $user['sex']            = $row['sex'];
        ...
        $user['qq']             = $row['qq'];
        $user['from_date']     = empty($row['from_date'])?'':local_date("Y-m-d",$row['from_date']);
        $user['to_date']     = empty($row['to_date'])?'':local_date("Y-m-d",$row['to_date']);
        $user['msn']            = $row['msn'];
...
elseif ($_REQUEST['act'] == 'export')
{
	...
	$sql = "SELECT user_id, email,user_name, user_money,rank_points,msn,mobile_phone FROM " . $ecs->table('users') ;
	...
	$PHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(20);//移动电话
	...
	$PHPExcel->getActiveSheet()->setCellValue('G3', '移动电话');
	
	$hang = 4;
	
	foreach ($alluser as $user_p ){
		
		$PHPExcel->getActiveSheet()->setCellValue('A' . ($hang), $user_p['user_id']." ");//加个空格，防止时间戳被转换
		$PHPExcel->getActiveSheet()->setCellValue('B' . ($hang), $user_p['msn']);
		$PHPExcel->getActiveSheet()->setCellValue('C' . ($hang), $user_p['user_name']);
		$PHPExcel->getActiveSheet()->setCellValue('D' . ($hang), $user_p['email']);
		$PHPExcel->getActiveSheet()->setCellValue('E' . ($hang), $user_p['user_money']." ");
		$PHPExcel->getActiveSheet()->setCellValue('F' . ($hang), $user_p['rank_points']);
		$PHPExcel->getActiveSheet()->setCellValue('G' . ($hang), $user_p['mobile_phone']);
		
		$hang++;
	}
	//设置单元格边框
	$PHPExcel->getActiveSheet()->getStyle('A1:H'.$hang)->applyFromArray($styleArray);
	//设置自动换行
	$PHPExcel->getActiveSheet()->getStyle('A4:H'.$hang)->getAlignment()->setWrapText(true);
	//设置字体大小
	$PHPExcel->getActiveSheet()->getStyle('A4:H'.$hang)->getFont()->setSize(12);
	//垂直居中
	$PHPExcel->getActiveSheet()->getStyle('A1:H'.$hang)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	//水平居中
	$PHPExcel->getActiveSheet()->getStyle('A1:H'.$hang)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id,dinggou_way, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,last_login ".
