...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    $smarty->assign('FCKeditor',    returnfck('goods_desc',$goods['goods_desc']));
    $goods['riji'] = returnfck('riji',$goods['riji']);
    $goods['ways'] = returnfck('ways',$goods['ways']);
    /* 模板赋值 */
    $smarty->assign('code',    $code);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $is_bind_card = isset($_POST['is_bind_card']) ? intval($_POST['is_bind_card']) : '1';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (is_bind_card,riji,ways,goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES (".$is_bind_card.",'$_POST[riji]','$_POST[ways]','$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
...
elseif ($_REQUEST['act'] == 'toggle_bind_card')
{
    ...
    if ($exc->edit("is_bind_card = '$is_hot', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_hot=='2'?1:0);
    }
}
/*------------------------------------------------------ */
//-- 修改商品排序
