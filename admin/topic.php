...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    include_once(ROOT_PATH.'includes/fckeditor/fckeditor.php'); // 包含 html editor 类文件
     ...
     $smarty->assign('mode',   isset($_REQUEST['mode'])?1:0);
    $smarty->assign('ur_here',     $_LANG['09_topic']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if(isset($_REQUEST['upload'])){
        $topic_class_list = $_REQUEST['topic_class_list'];
        $topic_id = $_REQUEST['topic_id'];



        if ($_FILES['pic']['name'] && $_FILES['pic']['size'] > 0){
            /* 检查文件合法性 */
            if(!get_file_suffix($_FILES['pic']['name'], $allow_suffix))
            {
            ...
            $name .= '.' . end(explode('.', $_FILES['pic']['name']));
            $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;
            if (move_upload_file($_FILES['pic']['tmp_name'], $target))
            {
                $cat_image = DATA_DIR . '/afficheimg/' . $name;
            }
            $cat_img = $db->getOne("select cat_img from ".$ecs->table("topic")." where topic_id=".$topic_id);
            if($cat_img){
               $cat_img = unserialize($cat_img);
            }else{
                $cat_img = array();
            }
            $cat_img[$topic_class_list] = $cat_image;

            $db->query("update ".$ecs->table("topic")." set cat_img='".serialize($cat_img)."' where topic_id=".$topic_id);
            clear_cache_files();
            $links[] = array('href' => 'topic.php?act=edit&mode=1&topic_id='.$topic_id, 'text' =>  '返回专题编辑页');
            sys_msg('分类图片上传成功', 0, $links);
        }else{
            $links[] = array('href' => 'topic.php?act=edit&mode=1&topic_id='.$topic_id, 'text' =>  '返回专题编辑页');
            sys_msg('上传图片不能为空', 0, $links);
        }
    }
