...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    /* 取得订单付款方式  */
    $sql = "select pay_name FROM " . $ecs->table('order_info') . " WHERE order_id = '" . $delivery_order['order_id'] . "'";
    $delivery_order['pay_name'] = $db->getOne($sql);
    
    /* 取得订单付款时间 */
    
    $sql = "select pay_time FROM " . $ecs->table('order_info') . " WHERE order_id = '" . $delivery_order['order_id'] . "'";
    $delivery_order['pay_time'] = $db->getOne($sql);
    $delivery_order['pay_time'] = local_date($_CFG['time_format'], $delivery_order['pay_time']);
    ...
    if (isset($_GET['print']))
    {
    	...
    	$smarty->template_dir = '../' . DATA_DIR;
    	$smarty->display('delivery_print.html');
    }
    else 
    {
    /* 显示模板 */
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        */
        ...
        file_put_contents(dirname(dirname(__FILE__))."/sms_log_".$_CFG['ihuyi_sms_user_name'].".log",$_CFG['ihuyi_sms_order_shipped'].' 收货人手机号：'.$sms_mobile."\r\n",FILE_APPEND);
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('consignee' == $step)
    {
        ...
        $_POST['best_time'] = $_POST['best_time'].' '.$_POST['best_time1'] ;
        $order = $_POST;
    ...
    elseif ('payment' == $step)
    {
        ...
        //$payment = available_payment_list(1,0);
    ...
    elseif ('money' == $step)
    {
        ...
        if (isset($_POST['finish']))
        {
            ...
            if ($step_act == 'add')
            {
                ...
                /* Added by Jack, 2015.07.11 */
                /* 获取用户手机号 */
                $sql = "SELECT user_id, mobile_phone FROM " . $ecs->table('users') . " WHERE user_id='$old_order[user_id]' LIMIT 1";
                $row = $db->getRow($sql);
                ...
                if ($_CFG['ihuyi_sms_customer_placed'] == '1' && $row['mobile_phone'] != '')
                {
                	...
                	$smarty->assign('shop_name',	$_CFG['shop_name']);
                	$smarty->assign('order_sn',		$old_order['order_sn']);
                	$smarty->assign('consignee',	$old_order['consignee']);
                	$smarty->assign('tel',			$old_order['tel']);
                	$smarty->assign('address',		$old_order['address']);
                	$smarty->assign('best_time',	$old_order['best_time']);
                
                	$content = $smarty->fetch('str:' . $_CFG['ihuyi_sms_customer_placed_value']);
                
                	$ret = sendsms($old_order['tel'], $content);
                }
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('consignee' == $step)
    {
        ...
        $btime = explode(" ",$order['best_time']);
        if(count($btime)==2){
            $order['best_time']     = $btime[0];
            $order['best_time1']     = $btime[1];
        }
        $smarty->assign('order', $order);
    }
...
//-- 处理
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'check_user_valiad')
{
    $uid = intval($_REQUEST['id']);
    $todate = $db->getOne("select to_date from ".$ecs->table("users")." where user_id=".$uid);
    if(!empty($todate) && $todate<=gmtime()){
        make_json_error('该用户合同期到期');
    }
    make_json_result('');
}
