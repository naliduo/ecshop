...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	for (var i = 0; i < 6; ++i, row = row.nextSibling) {
		...
		for (var j = 0; j < 7; ++j, cell = cell.nextSibling, date.setDate(iday + 1)) {
            ...
            var wWeek = new Date(date).getWeekNumber();
            var nowtime = document.getElementById("nowtime").value;
            var varWeek = new Date(nowtime).getWeekNumber();
            var varday = new Date(nowtime).getDay();
            var hrnow = today.getHours();
            
            
            

            // Added by Jack, 禁用部分日期
            /*if(wday==1 ||((varday == 6)&& wday == 0)||( ((wday-varday) ==1) && (wWeek == varWeek) )||date.print("%Y-%m-%d")<=nowtime ||date.print("%Y-%m-%d")=="2015-02-18" ){
                cell.disabled = true;
                cell.style.color='#c9c9c9';
			...
			*/
            ...
            if(wday==0 ||wday==1 ||((varday == wday) && (wWeek == varWeek))  ||(((wday-varday)==1) && (wWeek == varWeek) && (hrnow>=12))  ||date.print("%Y-%m-%d")<=nowtime ||date.print("%Y-%m-%d")=="2015-02-18" ){
                cell.disabled = true;
                cell.style.color='#c9c9c9';
