...
function checkConsignee()
{
 ...
 /* if (frm.elements['country'] && frm.elements['country'].value == 0)
  ...
  if (frm.elements['mobile'] && frm.elements['mobile'].value.length > 0 && (!Utils.isTel(frm.elements['mobile'].value)))
  {
   ...
   if (frm.elements['best_time'] && frm.elements['best_time'].value == '请选择'){
       err = true;
       msg.push("请选择最佳送货时间");
    }
  if (err)
  ...
  if(! err){
      var stime = document.getElementById("start_time_btn0").value;
...
function checksendtimeResponse(result){
     if(result.error>0){
         alert("由于农场配货需要，无法保证中午12点后下的订单次日能为您送达，为了不耽误您的宝贵时间，建议您选择次日以外的时间配送");
         return;
     }
    //alert(result.error);
